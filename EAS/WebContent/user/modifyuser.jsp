...
<body>
	<form role="form" action="../user/AddUserServlet">
		...
		<div class="checkbox">
			<label><input type="radio" value="2" id="author"
				name="author"
				<c:if test="${user.author=='2' }">checked="checked"</c:if> />学生</label> <br>
			<br> <label><input type="radio" value="1" id="author"
				name="author"
				<c:if test="${user.author=='1' }">checked="checked"</c:if> />老师</label> <br>
			<br> <label><input type="radio" value="1" id="author"
				name="author"
				<c:if test="${user.author=='0' }">checked="checked"</c:if> />管理员</label>
		</div>
